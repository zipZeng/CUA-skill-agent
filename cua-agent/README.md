...
- **意图解析** — 自然语言 → 结构化意图，支持中英文混合指令
- **任务规划** — 模板库匹配 + 步骤生成，支持扩展自定义应用
- **非独占式操控** — 激活窗口 → 执行操作 → 释放焦点，不影响用户正常工作
- **OCR 定位** — 截图 → 文字识别 → 坐标定位，无需依赖 UI Automation
- **桌面图标启动** — 显示桌面 → OCR 扫描图标 → 双击启动（开始菜单兜底）
...
intent_parser.py     意图解析（关键词正则 + Ollama 模型兜底）
task_planner.py      任务规划（模板库匹配 → Step 序列）
action_executor.py   动作执行（逐步执行，日志 + 重试 + 可选步骤跳过）
window_manager.py    窗口管理（查找/激活/启动/截图/键鼠操作）
...
|------|------|----------|
| `launch` | 启动应用 | `text` |
| `click` | OCR 定位 → 左键点击 | `target`, `fallback` |
| `right_click` | OCR 定位 → 右键点击 | `target`, `fallback` |
| `double_click` | OCR 定位 → 双击 | `target`, `fallback` |
| `type` | 输入文字（剪贴板粘贴） | `text` |
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `ocr_fuzzy_threshold` | 0.6 | OCR 模糊匹配阈值 |
