...
AGENT_SYSTEM_PROMPT = """你是一个 Windows 桌面自动化助手。你能"看到"屏幕上的文字及其坐标和区域标签，根据用户目标逐步操作。
...
区域标签含义:
- [菜单] — 窗口顶部菜单栏/工具栏，不要点击这里的功能入口
- [侧栏] — 左侧导航栏，x < 200，只用于切换页面，不是数据内容
- [主区] — 页面主体内容区，你要操作的大部分目标在这里
- [右区] — 右侧面板，通常是辅助信息
...
1. **严格按照用户目标中的文字操作**。用户说"点击游客登录"，就找"游客登录"，不要选"通行证登录"；用户说"沪深京排行"，就找"沪深京排行"，不要只找"沪深京"
2. target 必须从屏幕文字中精确选择，不要编造或缩短
3. **[菜单] 和 [侧栏] 区域不要点击**，除非用户明确要求"切换标签页"或"点击菜单"
4. 找不到目标文字时，先 scroll 向下滚动再找，不要选相近但不对的文字
5. 弹窗/广告优先关闭（找"关闭""×""取消""我知道了"）
        ...
        for step_num in range(1, MAX_STEPS + 1):
            ...
            for text, (x1, y1, x2, y2) in all_texts:
                ...
                if cy < 50:
                    ...
                    region = "侧栏"
                elif cx > screen_w - 300:
                    region = "右区"
                else:
                    region = "主区"
                ocr_lines.append(f'  [{region}] "{text}" @({cx}, {cy})')
            ocr_block = "\n".join(ocr_lines)
